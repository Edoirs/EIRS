...
import kotlinx.android.synthetic.main.activity_unsynced_poa.*
import kotlinx.android.synthetic.main.row_unsynced_poa.view.*
...
class UnsyncedPoaActivity : BaseAppCompatActivity() {
...
//                toast("Some error occurred while syncing, please re-sync")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_unsynced_poa)
...
//        }
    ...
    private fun performSync() {
        ...
        poaSyncList.add(Syncable(Submission::class.java.simpleName, "Captured MPoA", true))
    ...
    private inner class ListAdapter(list: ArrayList<Submission>) : BaseRecyclerViewAdapter<Submission, ListAdapter.ViewHolder>(list) {
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
            val v = LayoutInflater.from(parent.context).inflate(R.layout.row_unsynced_poa, parent, false)
            return ViewHolder(v)
        }
...
//            )
            ...
            holder.itemView.tvAmount.text = "${getString(R.string.niara)}${item.amountPaid.format(2, true)}"
            holder.itemView.tvDate.text = Date(item.submissionTime).format("dd MMM, yyyy hh:mm:ss aa")
            holder.itemView.tvCaptureDetail.text = "${item.menuSection}, ${item.group}, ${item.subGroup}, ${item.category}"
            holder.itemView.tvTaxPayerName.text = item.name

            val lga = SQLite.select().from(Lga::class.java)
                    .where(Lga_Table.areaId.eq(item.areaId))
                    .querySingle()

            holder.itemView.tvLga.text = lga?.areaName ?: ""
        }
